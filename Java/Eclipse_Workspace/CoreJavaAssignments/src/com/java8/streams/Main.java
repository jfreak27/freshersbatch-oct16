...
public class Main {
	...
	static void testNews() {
		...
		String username = newsList.stream().collect(groupingBy(News::getCommentByUser, counting())).entrySet().stream().max(Map.Entry.comparingByValue()).get().getKey().toString();
		System.out.println("User with Max Comments "+username);
	}
	...
	static void testTransactions() {
		...
		System.out.println("Transaction with MAX value "+value);
		
		
		//14.	Find the transaction with the smallest value.
		
		String str1 = transactionList.stream().min(Comparator.comparing(Transaction::getValue)).toString();
